...
### Continuing via Claude Code (2026-09, round 3) — tech-debt cleanup
- `@app.on_event("startup"/"shutdown")` → `lifespan` context manager ✅ — `server.py`, same index/backfill/scheduler logic on the way up, same client/http cleanup on the way down, just re-wired per FastAPI's current recommendation (on_event is deprecated).
- `shadow*` → `boxShadow` ✅ — replaced the deprecated `shadowColor`/`shadowOffset`/`shadowOpacity`/`shadowRadius` RN style props (was logging a console warning on every screen) with the CSS `boxShadow` string, in the shared `shadow.card`/`shadow.soft` theme presets (`src/theme/index.ts`, used by ~16 files) plus two one-off shadows in `LandingPage.tsx`/`account.tsx` that didn't go through the theme. `elevation` (Android) untouched, not part of the deprecation.

## Backlog (next phases)
- None open. Everything tracked in this file is done.
